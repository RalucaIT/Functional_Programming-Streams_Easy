...
public class Pack_3_Streams_Easy {
    ...
    public void example_1_forEach() {
        ...
        names.stream().forEach(name -> System.out.println("Hello " + name + "!")); // -> prints (the elements) the Parameter name (type Predicate) in the list.
    } // with Streams I can more easily use Lambda Functions to manipulate data, and access forEach statements, for eg.
    ...
    public void exercise_1_map_collect() {
        // divide each number by 2 (ignoring the reminder/rest) and collect to set = Methods.

        List<Integer> numbers = asList(1, 2, 3, 4, 5);
        Set<Integer> result = null; // Set declared, which keeps ONLY one single item/elem. DO NOT duplicate them.

        result = numbers.stream() // we make a stream from our list, so we can use the Stream operations.
                .map(n -> n / 2) // Lambda expression (atribuie lui n calculul cerut)
                .collect(toSet());

        System.out.println(result);

       assertThat(result, sameBeanAs(ImmutableSet.of(0, 1, 2))); // here Compares if the result is correct.
    }
    ...
    public void exercise_2_map() {
        // find all distinct reminders/rest/results of division by 13 and collect them sorted

        List<String> strings = asList("9", "4", "5", "6", "2", "3", "9", "4", "3", "4", "5");
        List<Integer> result = null;

        result = strings.stream() // .stream converts the List into a Stream (which is a different data)
        .map(n -> Integer.valueOf(n) % 13) // just transformed the String into int, to be able to do the Division
        .distinct() // selected without Duplicates
        .sorted()
        .collect(toList());
    ...
    @Test // Ciprian a facut ex. in prezentare.
    public void example_5_reduce() {
        Stream.of(1, 2, 3, 4).reduce((a, b) -> a + b).ifPresent(System.out::println); // Method Reference

        Stream<Integer> emptyStream = Stream.empty();
        Optional<Integer> optionalResult = emptyStream.reduce((a, b) -> a + b); // reduce to one single result.
    ...
    public void exercise_3_reduce() {
        ...
        result = strings.stream()
                .mapToInt(n -> n.length())
                .reduce(0, (a, b) -> a >= b ? a : b); // reduce a OR b
...
//        sum = list.stream().sum(); // doesn't compile, not an int stream
    ...
    public void exercise_4_filter_limit() {
        ...
        IntStream numbers = new Random(0).ints().map(Math::abs); // Method Reference
        List<Integer> result = null;
    ...
    public void exercise_6_toArray() {
        ...
        for (Integer integer : result) { // this is the sintax to itterate through an Array.
